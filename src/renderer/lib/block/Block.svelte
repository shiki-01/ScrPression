<script lang="ts">
    ...
    const content: Writable<BlockType> = writable(blockStore.getBlock(id));
...
<div
    ...
    use:useDrag={{
        ...
        onStart: () => {
            isDragging = true;
            EventManager.emit('block:drag:start', $content.id);
        },
        onEnd: () => {
            isDragging = false;
            EventManager.emit('block:drag:end', $content.id);
        },
        onDrag: (e) => {
            if (isDragging) {
                EventManager.emit('block:drag', { id: $content.id, x: e.clientX, y: e.clientY });
            }
        },
    }}
    on:pointerdown={(e) => {
