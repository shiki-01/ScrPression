...
import { useBlocksStore, draggingStore } from '$lib/store';
...
const List: React.FC<ListProps> = ({ content }) => {
  ...
  const addBlock = () => {
    ...
    useBlocksStore.getState().addContent({ ...content, id: newId });
    draggingStore.setState({ id: newId });
    console.log('Added block:', newId);
  };
    ...
    >
      ...
      >
        <div className="absolute left-0 top-0 -z-10 h-0 w-full">
          <svg className="" height={listHeight} role="none" width={listWidth + 2} xmlns="http://www.w3.org/2000/svg">
            <path
              d={isFlag
