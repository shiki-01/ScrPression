...
const Drag: React.FC<DragProps> = ({ content, initialPosition, onEnd }) => {
    ...
    useEffect(() => {
        const onMouseMove = (event: MouseEvent) => {
            setPosition({ x: event.clientX - offsetX.current, y: event.clientY - offsetY.current });
        ...
        const onMouseUp = (event: MouseEvent) => {
            ...
            document.elementsFromPoint(event.clientX, event.clientY).forEach((element) => {
                if (element.classList.contains('trash') || element.classList.contains('block-list')) {
                    console.log('Deleted block:', content.id);
                    useBlocksStore.getState().removeContent(content.id);
                }
            });
    ...
    }, [initialPosition, onEnd, position]);
