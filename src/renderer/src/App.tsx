...
import { useBlocksStore, blockListStore, draggingStore } from '$lib/store';
...
const App: React.FC = () => {
  ...
  const blocks = useBlocksStore((state) => state.contents);

  const lists = blockListStore((state) => state.blocklist);

  useEffect(() => {
    blockListStore.setState({ blocklist: contents });
  }, []);

  const [dragging, setDragging] = useState(draggingStore.getState().id);
  const [initialPosition, setInitialPosition] = useState({ x: 0, y: 0 });

  useEffect(() => {
    const unsubscribeDragging = draggingStore.subscribe((state) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex h-full w-full flex-row justify-between bg-slate-500 px-5">
        <div className="flex flex-row gap-4">
          <div className="h-full w-[100px]">
            <img src="https://placehold.jp/200x200" alt="logo" className="h-full w-full object-cover" />
          </div>
      ...
      <div
        onPointerDown={() => { setIsContextMenuOpen(false); }}
        className="grid grid-cols-[250px_1fr] grid-rows-1 overflow-hidden"
      >
        <div
          className="block-list relative flex h-full w-full select-none flex-col items-start gap-5 bg-slate-200 p-5"
        >
          {lists.map((list, i) => (
        ...
        <div className="grid grid-rows-[1fr_250px]">
          <div className="relative h-full w-full overflow-hidden">
            <div
              ...
              className="canvas w-full h-full relative cursor-grab bg-slate-50 active:cursor-grabbing"
            >
              {dragging && (
                <Drag
                  content={blocks.get(dragging)!}
                  initialPosition={{
                    x: blocks.get(dragging)!.position.x + 250,
                    y: blocks.get(dragging)!.position.y + 50
                  }}
                  onEnd={(position) => {
                    useBlocksStore.getState().updateContent(dragging, { position: { x: position.x - 250, y: position.y - 50 } });
                    draggingStore.setState({ id: '' });
                  }}
                />
              )}
              {Array.from(blocks.entries()).map(([_, content], i) => (
                dragging !== content.id && (
            ...
            <div
              className="trash absolute bottom-2 right-2 text-slate-400 transition-colors duration-300 hover:text-slate-500"
            >
            ...
            <div className="absolute right-6 top-6 flex flex-col gap-2">
              <button
                className="flex items-center justify-center rounded-full border-2 border-slate-400 bg-slate-50 text-slate-400"
              >
